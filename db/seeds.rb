...
message = Message.create(
  ...
  body: "Welcome to Slock, a clone of Slack that has (almost) all of its key features!<br><ol><li>View user information by clicking on their icons in chat, or click on your own icon in the upper right to view and edit your information.</li><li>Hover over messages to add reacts, save, edit, or delete.</li><li>Access the channel, people, and saved items browsers at the top of the workspace sidebar on the left.</li><li>Send richly formatted messages and upload files using our versatile text editor.</li><li>View channel details such as files, description, topic, members, and more by clicking on the channel name above.</li><li>Start video chats with people in your direct messages through our WebRTC video chatting service. Disclaimer: as there is no TURN server set up, both users must be on the same wifi network.</li></ol>"
)
MessageReact.create(
